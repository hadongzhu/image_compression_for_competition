...
bool Matchfinder_base::read_block()
{
    if (!at_stream_end && stream_pos < buffer_size)
    {
        ...
        const int rd = size < inputStreamLength ? size : inputStreamLength;
        memcpy(buffer + stream_pos, inputStream, rd);
        stream_pos += rd;
        if (rd < size)
        {
            at_stream_end = true;
      ...
      at_stream_end(false)
{
    ...
    size += pos_array_size;
    if (size * sizeof prev_positions[0] <= size)
        prev_positions = 0;
    else
