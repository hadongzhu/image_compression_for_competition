                                   ...
                                   const State state)
{
    ...
    if (main_len > replens[0])
    {
        ...
        while (true)
        {
            ...
            if (++len > pairs[i].len && ++i >= num_pairs)
                break;
        }
    }
    ...
    while (true) // price optimization loop
    {
        ...
        State cur_state;
        {
            ...
            if (prev_index2 == single_step_trial)
            {
                ...
                {
                    if (dis4 == 0)
                        cur_state.set_short_rep();
                    else
                        cur_state.set_char(); // literal
                }
        ...
        if (match_byte != cur_byte && next_trial.prev_index != cur)
        {
            ...
            int len = 1;
            while (len < limit && data[len - dis] == data[len])
                ++len;
        ...
        for (int rep = 0; rep < num_rep_distances; ++rep)
        {
            ...
            while (len2 < limit && data[len2 - dis] == data[len2])
                ++len2;
            len2 -= len + 1;
        ...
        if (newlen >= start_len && newlen <= len_limit)
        {
            ...
            for (int len = start_len;; ++len)
            {
                ...
                if (len == pairs[i].len)
                {
                    ...
                    int len2 = len + 1;
                    const int limit = std::min(match_len_limit + len2, triable_bytes);
                    while (len2 < limit && data[len2 - dis2] == data[len2])
                        ++len2;
                    len2 -= len + 1;
                    if (len2 >= min_match_len)
                    {
